 ...
 * via the property-tag mailbox interface (drivers/firmware/bcm2835) and
 * writes pixels into the returned memory.
 *
 ...
 *
 * The framebuffer lives in VideoCore-shared DRAM. ALLOCATE_BUFFER
 * returns a VC-bus address in the 0xC0000000-alias range; the ARM
 * physical address is `bus & 0x3FFFFFFF`. Mapped K_MEM_CACHE_NONE so
 * CPU writes land in the buffer without separate cache maintenance,
 * matching VC's uncached scanout view.
 */
...
static int bcm2835_fb_init(const struct device *dev)
{
	 ...
	 * `vcgencmd get_lcd_info` reports under Linux.
	 */
	if (w == 0 || h == 0) {
		err = bcm2835_property_fb_get_size(&w, &h);
		if (err < 0) {
	...
	err = bcm2835_property_fb_setup(&w, &h, &depth, &order,
