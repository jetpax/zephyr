 ...
 *
 * Polled-first mirrors the sibling sdhc_bcm2835.c: it keeps bring-up
 * debuggable, and for display-class traffic a tight byte-pump keeps
 * the FIFO full without IRQ overhead. Linux's spi-bcm2835.c likewise
 * runs short transfers in a polling path. IRQ / DMA can be layered on
 * later behind the same transceive() entry point.
 *
 * Silicon notes:
 *   - Chip select is always GPIO-backed (cs-gpios); the controller's
 *     native CE0/CE1 lines are not used. Native CE ties CS assertion
 *     to FIFO activity, which is awkward for displays that hold CS
 *     across a D/C-pin toggle. GPIO CS is also the conventional Zephyr
 *     pattern and what display_st7789v expects.
 *   - SCLK = core_clock / CDIV, CDIV even. The 2012 datasheet says
 *     "power of 2", but BCM2835-and-later silicon accepts any even
 *     value (Linux relies on this). CDIV == 0 encodes 65536.
...
static int spi_bcm2835_init(const struct device *dev)
{
	 ...
	 * roles can be board-manifest dependent (the layered board-config
	 * model). When the property is absent, cfg->pcfg is NULL.
	 */
	if (cfg->pcfg != NULL) {
