...
static const char *armv6_fsr_str(uint32_t fsr)
{
	...
	case 0x4: return "instruction cache maintenance fault";
	case 0x5: return "translation fault (section)";
	case 0x7: return "translation fault (page)";
	case 0x8: return "external abort";
...
bool z_arm_fault_data(struct arch_esf *esf)
{
	...
	printk("ARMv6 data abort: %s on %s (DFSR 0x%08x, DFAR 0x%08x)\n",
	       armv6_fsr_str(dfsr), (dfsr & BIT(11)) ? "write" : "read",
	       dfsr, dfar);
	z_arm_fatal_error(K_ERR_CPU_EXCEPTION, esf);
	return true;
}
