			     ...
			     uintptr_t tls_value)
{
	...
	/* Optional hardening (verdict #2): a future sketch defining its OWN extra
	 * TLS var would alias past the loader's TLS block. Catch it loudly:
	 * __ASSERT(tls_value + rel->r_addend < z_tls_data_size(), "...");  // kernel_tls.h
	 */
